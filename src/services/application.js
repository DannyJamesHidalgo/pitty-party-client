export const postNewApplication = async (applicationData) => {
    try {
      ...
      const response = await fetch("https://lobster-app-bbl8x.ondigitalocean.app/applications", {
        method: 'POST',
        headers: {
...
export const getAllApplicationsByAdopterId = (adopter_id) => {
  return fetch(`https://lobster-app-bbl8x.ondigitalocean.app/applications?adopter_id=${adopter_id}`, {
    headers: {
      'Content-Type': 'application/json',
      Authorization: `Token ${JSON.parse(localStorage.getItem("pitty_token")).token}`,
    },
...
export const getApplicationById =(App_id)=>{
  return fetch(`https://lobster-app-bbl8x.ondigitalocean.app/applications/${App_id}`, {
    headers: {
      'Content-Type': 'application/json',
      Authorization: `Token ${JSON.parse(localStorage.getItem("pitty_token")).token}`,
    },
...
export const deleteApplicationById = (appId) => {
  return fetch(`https://lobster-app-bbl8x.ondigitalocean.app/applications/${appId}`, {
    method: 'DELETE',
    headers: {
      'Content-Type': 'application/json',
      Authorization: `Token ${JSON.parse(localStorage.getItem("pitty_token")).token}`,
